...
xrdp_encoder_create(struct xrdp_mm *mm)
{
    ...
    if (mm->egfx_flags & 1)
    {
        LOG(LOG_LEVEL_INFO, "xrdp_encoder_create: starting h264 codec session gfx");
    ...
    {
        LOG(LOG_LEVEL_INFO, "xrdp_encoder_create: starting gfx rfx pro codec session");
        self->in_codec_mode = 1;
...
process_enc_rfx(struct xrdp_encoder *self, XRDP_ENC_DATA *enc)
{
    ...
    if ((enc->num_crects > 0) && (enc->num_drects > 0))
    {
        ...
        if (out_data != NULL)
        {
            ...
            LOG_DEVEL(LOG_LEVEL_TRACE, "process_enc_rfx: num_crects %d num_drects %d",
                   enc->num_crects, enc->num_drects);
...
process_enc_h264(struct xrdp_encoder *self, XRDP_ENC_DATA *enc)
{
    ...
    if (error != 0)
    {
        LOG_DEVEL(LOG_LEVEL_TRACE, "process_enc_h264: xrdp_encoder_x264_encode failed rv %d",
               error);
        g_free(out_data);
        return 0;
    }
